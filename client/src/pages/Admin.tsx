...
function GuestFormPanel({
  ...
  guestForm: GuestFormState;
  setGuestForm: Dispatch<SetStateAction<GuestFormState>>;
  phoneDialCode: string;
...
export default function Admin() {
  ...
  function openEditGuestDialog(guest: GuestRecord) {
  ...
  return (
    <main className="min-h-screen bg-background px-6 py-10 md:px-10 md:py-14">
      <Dialog open={isGuestDialogOpen} onOpenChange={(open) => (!open ? closeGuestDialog() : setIsGuestDialogOpen(true))}>
        <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto rounded-none border border-primary/10 p-0">
          <div className="p-8 md:p-10">
            <DialogHeader className="mb-8">
              ...
              <DialogTitle className="mt-3 font-serif text-3xl text-foreground md:text-4xl">
                {editingGuestId
                  ? "Mettez à jour les informations de l'invité"
                  : "Créer une invitation personnalisée"}
              </DialogTitle>
              <DialogDescription className="mt-3 text-sm leading-7 text-foreground/70">
                Renseignez l'invité, son RSVP et ses coordonnées dans un espace dédié.
              </DialogDescription>
            </DialogHeader>
            ...
            <GuestFormPanel
              ...
              onSubmit={() => saveGuestMutation.mutate()}
              isSubmitting={saveGuestMutation.isPending}
            />
          </div>
        </DialogContent>
        ...
        <div className="mx-auto max-w-7xl space-y-10">
          ...
          <section className="grid gap-8 xl:grid-cols-[0.34fr_1.66fr]">
            <article className="border border-primary/10 bg-white p-6 editorial-shadow md:p-8">
              <p className="text-[11px] uppercase tracking-[0.45em] text-primary/60">
                Gestion
              </p>
              <h2 className="mt-3 font-serif text-3xl text-foreground md:text-4xl">
                Créer et piloter les invitations.
              </h2>
              <p className="mt-4 text-sm leading-7 text-foreground/70">
                L'ajout et la modification d'un invité se font maintenant dans
                un dialogue dédié pour garder la liste plus légère.
              </p>
              <Button
                type="button"
                onClick={openCreateGuestDialog}
                className="mt-8 rounded-none bg-primary px-6 text-[10px] uppercase tracking-[0.35em] text-primary-foreground hover:bg-foreground"
              >
                <Plus className="mr-2 h-4 w-4" strokeWidth={1.6} />
                Créer un invité
              </Button>
                            ...
                            <TableCell className="min-w-[290px] py-6 text-right">
                              <div className="flex flex-wrap justify-end gap-2">
                                ...
                                <Button
                                  ...
                                  size="sm"
                                  variant="outline"
                                  onClick={() => openEditGuestDialog(guest)}
                                  className="rounded-none border-primary/15 text-[10px] uppercase tracking-[0.25em] text-primary"
                                >
